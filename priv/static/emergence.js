...
(function () {
  ...
  micBtn.addEventListener('click', async () => {
    ...
    setStatus(T('listening — speak, then pause to send'), true);
    startVad(stream);
  });
  /* Decode a recorded blob to a 16 kHz mono Float32Array for on-device STT. */
  async function blobToPcm16k(blob) {
    ...
    if (rms > 0.003 && rms < 0.1) {
        const g = Math.min(0.1 / rms, 30);
        for (let i = 0; i < pcm.length; i++) pcm[i] *= g;
    }
    /* Soft limiter (tanh): tame the odd transient without a global peak scale
     * (which suffocated quiet speech) or hard clipping (which made whisper
     * loop). Near-linear for speech-level samples, smoothly squashes spikes. */
    for (let i = 0; i < pcm.length; i++) pcm[i] = Math.tanh(pcm[i]);
    return pcm;
  }
})();
